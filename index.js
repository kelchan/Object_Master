...
const namesPoisonType = pokémon.filter( p => p.types.length < 2 && p.types[ 0 ] == 'poison' ).map( p => p.name );
console.log( namesPoisonType );

const secondTypeFlying = pokémon.filter( p => p.types[ 1 ] == 'flying' ).map( p => p.types[ 0 ] );
console.log( secondTypeFlying );

const numOfNormalType = pokémon.filter( p => p.types.includes( 'normal' ) ).length;
console.log( numOfNormalType );